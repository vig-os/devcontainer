...
- **Prepare-release extension hook (`prepare-release-extension.yml`)** ([#1059](https://github.com/vig-os/devkit/issues/1059))
  ...
  - Devkit dogfoods the hook: its own `prepare-release-extension.yml` runs the workspace-manifest sync that was a hardcoded divergence in `prepare-release.yml`, making the prepare workflow scaffold-shaped again.
