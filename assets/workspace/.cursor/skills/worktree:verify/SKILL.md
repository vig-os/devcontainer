...
Run full verification and provide evidence **without user interaction**. This is the worktree variant of [code:verify](../code:verify/SKILL.md). On failure, loop back to fix.
...
If stuck after 3 attempts on the same failure, use [worktree:ask](../worktree:ask/SKILL.md) to post a question on the issue.

### 4. Proceed to PR

Once all checks pass, invoke [worktree:pr](../worktree:pr/SKILL.md) to create the pull request.
