...
- **Commit the failing test** using [commit-msg](../commit-msg/SKILL.md) with type `test`, e.g. `test: add failing test for <behavior>`. This creates an auditable record that the test was written first.
...
- **Commit the implementation** using [commit-msg](../commit-msg/SKILL.md), e.g. `feat: implement <behavior>`.
...
- **Commit the refactor** using [commit-msg](../commit-msg/SKILL.md) with type `refactor`, if there are meaningful changes. Skip if nothing changed.
