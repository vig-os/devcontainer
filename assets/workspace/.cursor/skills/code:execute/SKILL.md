...
4. If no comment contains `## Implementation Plan`, **stop** and tell the user to run [design:plan](../design:plan/SKILL.md) first.
...
- For each task:
  ...
  2. Implement the change (following [coding-principles](../../rules/coding-principles.mdc) and TDD). Commit each phase via [git:commit](../git:commit/SKILL.md).
...
- Diagnose using [code:debug](../code:debug/SKILL.md) principles if needed.
...
- Report final status.
- Suggest committing and proceeding to [pr:create](../pr:create/SKILL.md).
