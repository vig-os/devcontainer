...
Poll CI pipeline status and react **without user interaction**. This is the worktree variant of [ci:check](../ci:check/SKILL.md) — it waits for CI to finish and auto-triggers fixes instead of reporting status and stopping.
...
3. If any check is still pending:
   ...
   - Maximum total wait: **15 minutes**. If still pending after 15 minutes, post a note via [worktree:ask](../worktree:ask/SKILL.md) and stop.
...
3. Invoke [worktree:ci-fix](../worktree:ci-fix/SKILL.md) with the failure context.
