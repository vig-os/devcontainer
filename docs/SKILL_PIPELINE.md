...
| Recipe | What it does |
|--------|-------------|
| `just worktree-start <issue> "<prompt>"` | Creates a worktree, resolves (or creates) the linked branch, sets up the environment (`uv sync`, `prek install`, `.env` copy), then launches a `tmux` session running `claude --dangerously-skip-permissions` with the given prompt. If the worktree already exists, it reuses it. |
...
4. **Worktree setup** — `git worktree add`, then inside the worktree: `uv sync`, `prek install`, copies `.env` from the main worktree.
