...
projects: none
relationship: none
synced: 2026-03-07T04:05:46.113Z
---
...
Implements a `just devc-remote` recipe that orchestrates starting a devcontainer on a remote host and connecting Cur*or/VS Code to it in a single command. This enables developers to spin up their devcontainer on powerful remote machines (GPU servers, cloud VMs) from their local terminal without manual SSH and compose steps.
...
- `scripts/devc_remote_uri.py`: Python helper for constructing Cur*or/VS Code nested authority URIs with hex-encoded devcontainer specs
...
- **scripts/devc-remote.sh** (249 lines): Main orchestrator script
  ...
  - `detect_editor_cli()`: Auto-detects `cur*or` or `code` CLI
  ...
  - `open_editor()`: Reads `workspaceFolder` from remote devcontainer.json and constructs Cur*or URI
...
- **devc_remote_uri.py — Cur*or URI construction for remote devcontainers** ([#153](https://github.com/vig-os/devcontainer/issues/153))
...
- **Cur*or nested authority URIs**: Uses hex-encoded devcontainer specs for single-command remote devcontainer opening
