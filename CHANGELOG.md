...
- **Compose tooling prefers `podman-compose` over docker-compose bridge** ([#70](https://github.com/vig-os/devcontainer/issues/70))
  - `podman-compose` (Python) correctly passes `devices`/`cap_add` to podman CLI
  - `docker-compose` v5.1.0 as podman external provider drops these fields (known bug)
  - Remote preflight now detects `podman-compose` > `podman compose` > `docker compose`
