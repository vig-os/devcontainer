...
* the PoC actually boots MinIO + Postgres as native processes — no Docker or
  Podman daemon — and tears down cleanly.
...
# Ports the PoC binds (flake.nix servicesPoC): non-default to dodge CI runners'
# own postgres (5432) and anything squatting on MinIO's usual 9000.
...
def test_services_poc_boots_minio_and_postgres(tmp_path: Path) -> None:
    """The PoC boots MinIO + Postgres as native processes and tears down clean.
    ...
    TCP socket and MinIO's liveness endpoint. No podman/docker fixture is used
    anywhere — that absence is itself the "no container daemon" assertion.
    The printed timings feed the #795 eval-cost/timing note.
    """
    ...
    try:
        ...
        _wait_for_http_ok(
            f"http://127.0.0.1:{POC_MINIO_PORT}/minio/health/live", deadline
        )
